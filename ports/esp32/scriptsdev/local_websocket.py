from uwebsocket import websocket  # type: ignore
...
class LocalWebSocketProcess:
    def __init__(self, waterbot:Waterbot, port=8266):
        self._socket = None
        self._port = port
        self._ws = None
        self._runTest = False
    ...
    def socketConnect(self):
        ...
        for i in (network.AP_IF, network.STA_IF):
            ...
            if iface.active():
                print(
                    "Websocket started on ws://%s:%d"
                    % (iface.ifconfig()[0], self._port)
                )
